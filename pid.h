...
struct pid_controller {
	...
	_iq * input; //!< Current Process Value
	_iq * output; //!< Corrective Output from PID Controller
	_iq * setpoint; //!< Controller Setpoint
	// Tuning parameters
	_iq Kp; //!< Stores the gain for the Proportional term
	_iq Ki; //!< Stores the gain for the Integral term
	_iq Kd; //!< Stores the gain for the Derivative term
	// Output minimum and maximum values
	_iq omin; //!< Maximum value allowed at the output
	_iq omax; //!< Minimum value allowed at the output
	// Variables for PID algorithm
	_iq iterm; //!< Accumulator for integral term
	_iq lastin; //!< Last input value for differential term
    _iq last_err;     
    _iq previous_err;
	// Time related
    ...
    enum pid_control_method pid_method;
...
extern "C" {
	 ...
	 * @param in Pointer to _iq value for the process input
	 * @param out Poiter to put the controller output value
	 * @param set Pointer _iq with the process setpoint value
